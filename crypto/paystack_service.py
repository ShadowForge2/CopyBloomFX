            ...
            if status == 'success':
                ...
                try:
                    ...
                    if deposit.status == 'pending':
                        ...
                        # Process referral commission for Paystack deposits
                        if deposit.referrer and deposit.referrer != deposit.user:
                            try:
                                from crypto.utils import REFERRAL_PCT
                                from crypto.models import Referral
                                
                                # Calculate 8% commission
                                bonus = deposit.amount_usdt * REFERRAL_PCT
                                ref_profile = deposit.referrer.profile
                                
                                if ref_profile:
                                    # Add commission to referrer's locked balance
                                    ...
                                    logger.info(f"Referral commission processed: {bonus:.2f} for {deposit.referrer.email}")
