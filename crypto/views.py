...
def signup_view(request):
    ...
    ref_code_from_url = request.GET.get('ref', '').strip()
    
    form = SignupForm(request.POST or None, initial={'referrer_code': ref_code_from_url})
    if request.method == 'POST' and form.is_valid():
        ip = get_client_ip(request)
        ...
        referrer = None
        ref_code = form.cleaned_data.get('referrer_code', '').strip()
        
        # Priority: Form data > URL parameter
        final_ref_code = ref_code or ref_code_from_url
        ...
        if final_ref_code:
            try:
                ...
                if referrer_profile.user != user:
                    ...
                    messages.success(request, f'You have been referred by {referrer.username}!')
            except Profile.DoesNotExist:
                messages.info(request, 'Referral code not found, but account created successfully.')
        ...
        profile = Profile.objects.create(
            user=user, 
            referral_code=code,
            referrer=referrer  # Store referrer permanently
        )
    ...
    # Pass referral code to template for display
    return render(request, 'crypto/signup.html', {
        'form': form, 
        'ref_code': ref_code_from_url
    })
...
def finance_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'deposit' in request.POST and deposit_form.is_valid():
            ...
            # Automatically use stored referrer from user profile
            ref = profile.referrer if profile else None
...
def referral_view(request):
    ...
    # Get recent referrals (limited to 10 most recent)
    recent_referrals = profile.user.referrals_made.select_related('referee').order_by('-created_at')[:10]
    
    # Get total referrals count for "View All" button
    total_referrals_count = profile.user.referrals_made.count()
    ...
    if request.method == 'POST':
        form = PromoRedeemForm(request.POST)
        if form.is_valid():
            ...
            # Create a deposit entry for promo bonus with 30-day expiration
            PromoRedemption.objects.create(
                user=request.user,
                promo_code=promo,
                bonus_amount=bonus,
                credited_to_locked=True
            )
...
def admin_deposit_approve_view(request, pk):
    ...
    if deposit_type == 'crypto':
        ...
        # Referral bonus for local deposits
        if d.referrer:
            bonus = d.amount_usdt * REFERRAL_PCT
            ref_profile = d.referrer.profile
            if ref_profile:
                ref_profile.locked_balance += bonus
                ref_profile.total_referrals += 1
                ref_profile.valid_referrals += 1
                ...
                Referral.objects.create(
                    ...
                    referee=d.user, 
                    bonus_amount=bonus, 
                    deposit=None  # Local deposit doesn't use Deposit model
                )
                ref_profile.update_rank()
                messages.success(request, f"Referral bonus of ${bonus:.2f} credited to {d.referrer.username}!")
